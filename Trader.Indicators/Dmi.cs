...
{
    public const int DefaultAdxPeriods = 14;
    public const int DefaultDiPeriods = 14;
    ...
    public Dmi(IndicatorResult<HLC> source, int adxPeriods = DefaultAdxPeriods, int diPeriods = DefaultDiPeriods)
        : base(source, x =>
        {
            ...
            var up = high.Change();
            var down = -low.Change();
            var plusDM = Indicator.Zip(up, down, (u, d) =>
            {
                if (u is null)
                {
                    return null;
            ...
            var minusDM = Indicator.Zip(up, down, (u, d) =>
            {
                if (d is null)
                {
                    return null;
            ...
            return Indicator.Zip(plus, minus, adx, (p, m, x) => new DMI(p, m, x));
        })
    {
    }
}
