...
public interface IKlineProvider
{
    ...
    /// </summary>
    ValueTask<ImmutableSortedSet<Kline>> GetKlinesAsync(string symbol, KlineInterval interval, CancellationToken cancellationToken = default);

    /// <summary>
    /// Returns the last <paramref name="periods"/> cached klines up to and including <paramref name="tickTime"/>.
    /// </summary>
    ValueTask<ImmutableSortedSet<Kline>> GetKlinesAsync(string symbol, KlineInterval interval, DateTime tickTime, int periods, CancellationToken cancellationToken = default);
